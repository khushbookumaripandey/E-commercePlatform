   ...
   <div class="container">
     <div class="row pt-5">
       ...
       <div class="col-md-4 col-12">
         <img :src="product.productImage" :alt="avtar" class="img-fluid">
       </div>
...
export default {
 ...
 methods:{
   addToCart(productId){
     axios.post("http://10.177.68.12:8085/product/cart/", {
         cart :  {
         cartId: localStorage.getItem('uId'),
 ...
 productId : this.product.productId,
   ...
   listCartItems(){
     axios.get(`http://10.177.68.12:8085/cart/${localStorage.getItem('uId')}`)
     .then((response) => {
       if(response.status===200){
 ...
 mounted() {
   console.log('i am show detail')
   this.id = this.$route.params.id;
    const path = `http://10.177.68.48:8083/product/${this.id}`
    axios.get(path)
      .then(res => {
